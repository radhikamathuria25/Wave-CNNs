...
class DWT_1D(Module):
    ...
    def get_matrix(self):
        ...
        self.matrix_low = torch.Tensor(matrix_h).to(self.device)
        self.matrix_high = torch.Tensor(matrix_g).to(self.device)
...
class DWT_2D_tiny(Module):
    ...
    def get_matrix(self):
        ...
        for i in range(L1 - L):
            for j in range(self.band_length):
                if index+j >= L1 + self.band_length - 2:
                    break;
                matrix_g[i, index+j] = self.band_high[j]
            index += 2
